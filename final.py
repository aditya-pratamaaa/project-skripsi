...
from sklearn.metrics import silhouette_score  # Library untuk menghitung Silhouette Score
...
from itertools import combinations  # Library untuk kombinasi pasangan klaster (Dunn Index)
...
# ============================================================================
# METRIK EVALUASI TAMBAHAN — Silhouette Score & Dunn Index
# (PC / Partition Coefficient sudah tersedia sebagai 'fpc' dari skfuzzy)
# ============================================================================
def dunn_index(data, labels, centers):
    """
    Menghitung Dunn Index sesuai Persamaan (6) pada BAB III:
    DI = min jarak antar-pusat klaster / max diameter klaster.
    Semakin besar nilainya, semakin baik pemisahan antar klaster.
    """
    unique_labels = np.unique(labels)
    if len(unique_labels) < 2:
        return np.nan

    # Jarak minimum antar pusat klaster (inter-cluster distance)
    inter_dists = [
    ...
    # Diameter maksimum tiap klaster (intra-cluster distance terbesar antar anggota)
    max_intra_diam = 0.0
...
def compute_cluster_metrics(data_scaled, cluster_labels, centers):
    """Menghitung Silhouette Score & Dunn Index untuk satu hasil clustering."""
    try:
...
m_fuzzy = 2.0
max_iter = 100        # disamakan dengan parameter eksperimen BAB III (Tabel 3.3)
error_tol = 0.00001   # disamakan dengan parameter eksperimen BAB III (Tabel 3.3)
RANDOM_SEED = 42
...
with tab_model:
    ...
    if 'data_scaled' not in st.session_state:
        ...
        # --- Metrik evaluasi tambahan: Silhouette Score & Dunn Index (selaras BAB III) ---
